...
func filename(m *uof.Message) string {
	...
	case uof.MessageKindEvent:
		...
		return fmt.Sprintf("/events/%d/%13d-%s", m.EventID, m.ReceivedAt, m.Type)
	case uof.MessageKindLexicon:
		switch m.Type {
		case uof.MessageTypePlayer:
			return fmt.Sprintf("/%s/players/%08d", m.Lang, m.Player.ID)
		case uof.MessageTypeMarkets:
			if len(m.Markets) > 1 {
				return fmt.Sprintf("/%s/markets/%s", m.Lang, m.Lang)
			}
			s := m.Markets[0]
			return fmt.Sprintf("/%s/markets/%08d-%08d", m.Lang, s.ID, s.VariantID)
		case uof.MessageTypeFixture:
			return fmt.Sprintf("/%s/fixtures/%08d", m.Lang, m.EventID)
		}
	case uof.MessageKindSystem:
		return fmt.Sprintf("/system/%13d-%s", m.ReceivedAt, m.Type)
	}
